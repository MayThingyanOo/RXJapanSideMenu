...
class CpsForm
{
    ...
    {
        $this->form_id = Request::input($this->input_name, Str::random(16));
    }
    ...
    public function keepExcept($keys)
    {
        ...
        if (!Request::has('file_upload_items') && array_key_exists('file_upload_items', $keepVal)) {
            unset($keepVal['file_upload_items']);
        }
        Session::put($this->form_id, $keepVal);
        view()->share([$this->getInputName() => $this->getFormId()]);
    }
    ...
    public function keepOnly($keys)
    {
        ...
        foreach ($keys as $key) {
            Arr::set($keepVal, $key, request($key));
        }
        Session::put($this->form_id, $keepVal);
        view()->share([$this->getInputName() => $this->getFormId()]);
    }
    ...
    public function input($key = null, $default = null)
    {
        if ($this->hasSessionFormId()) {
            $data = Session::get($this->form_id);
            return Arr::get($data, $key, request()->input($key, $default));
        }
