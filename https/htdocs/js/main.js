...
$(function () {
    ...
    // Remove DataTable Original Select Box Classes
    $(".dataTables_length .l-menu").children('select').removeClass('form-select form-select-sm').addClass('form-control md-input');

    // When the user clicks anywhere outside of the dt-button-collection, close it
    $(window).click(function (event) {
        if (event.target.className == "dt-button-background") {
            $('.buttons-colvis').removeClass("buttons-triangle-visible");
        }
    });

    //for triangle show/hide
    $('.buttons-colvis').on('click', function () {
        $(this).addClass("buttons-triangle-visible");
    });
});
...
$.fn.DataTable.defaults.oLanguage.sLengthMenu = "<span class='lengthMenu w_set'>表示件数</span><span class='l-menu'>_MENU_</span><span class='p-count'>件</span>";
